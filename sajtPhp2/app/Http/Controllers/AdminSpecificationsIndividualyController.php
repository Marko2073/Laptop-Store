...
use App\Http\Requests\SpecificationsIndiviRequest;
...
class AdminSpecificationsIndividualyController extends Controller
{
     ...
     */
    public function store(SpecificationsIndiviRequest $request)
    {
        $data = $request->input();
     ...
     */
    public function update(SpecificationsIndiviRequest $request, string $id)
    {
        $data = $request->input();
        if(isset($data['_token'])){
            unset($data['_token']);
    ...
    {
        DB::table('specifications_individually')->where('id', $id)->delete();
        return redirect()->route('table', ['name' => 'specifications_individually']);
    }
}
